...
import 'package:expense_manager/features/user/data/model/user_model.dart';
...
class UserService {
  ...
  Future<void> updateTotalExpense(String userId, double amount) async {
    try {
      await _firestore.collection('users').doc(userId).update({
  ...
  // Update user's total earning
  Future<void> updateTotalEarning(String userId, double amount) async {
    try {
      await _firestore.collection('users').doc(userId).update({
        'totalEarning': FieldValue.increment(amount),
      });
    } catch (e) {
      print('Error updating total earning: $e');
      rethrow;
    }
  }
}

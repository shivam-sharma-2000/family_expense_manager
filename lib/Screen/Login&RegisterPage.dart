...
class _LoginAndRegisterPageState extends State<LoginAndRegisterPage> {
  ...
  late final AuthService fService;
  // Controllers
  ...
  Future<void> _handleRegistration() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        var user = await fService.registerWithEmail(email: _emailController.text.trim(), password: _passwordController.text.trim());
        if(user != null){
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error during signIn: ${e.toString()}')),
        );
      } finally {
        if (mounted) {
          setState(() {
  ...
  @override
  void initState() {
    fService = sl<AuthService>();
    _firestore = sl<FirebaseFirestore>();
    initializeDB();
    super.initState();
  }
  ...
  Future<void> _saveUserToFirestore(User user) async {
    ...
    await doc.set({
      ...
      'email': user.email,
      'familyId': _familyIdController.text.isEmpty ? null : _familyIdController.text.trim(),
